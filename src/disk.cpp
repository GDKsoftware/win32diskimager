...
char *readSectorDataFromHandle(HANDLE handle, unsigned long startsector, unsigned long numsectors)
{
	...
	char *data = new char[512 * numsectors];
	SetFilePointer(handle, startsector * 512, NULL, FILE_BEGIN);
	if (!ReadFile(handle, data, 512 * numsectors, &bytesread, NULL))
	{
		char errormessage[512];
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &errormessage[0], 0, NULL);
...
bool writeSectorDataToHandle(HANDLE handle, char *data, unsigned long startsector, unsigned long numsectors)
{
	...
	BOOL bResult;
	SetFilePointer(handle, startsector * 512, NULL, FILE_BEGIN);
	bResult = WriteFile(handle, data, 512 * numsectors, &byteswritten, NULL);
	if (!bResult)
	{
		char errormessage[512];
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &errormessage[0], 0, NULL);
